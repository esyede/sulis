...
```php
$v = Sulis::validator()->make(array('name' => 'Chester Tester'));
...
```php
$v = Sulis::validator()->make(array('settings' => array(
    array('threshold' => 50),
    array('threshold' => 90)
)));
...
```php
$v = Sulis::validator()->make(array('values' => array(50, 90)));
...
```php
$v = Sulis::validator()->make(array('user' => array('first_name' => 'Steve', 'last_name' => 'Smith', 'username' => 'Batman123')));
...
$v = Sulis::validator()->make(['name' => 'John']);
$v->rule('required', ['name']);
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['username' => 'spiderman', 'password' => 'Gr33nG0Blin', 'required_but_null' => null]);
$v->rules([
    'required' => [
        ['username'],
        ['password'],
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['username' => 'spiderman', 'password' => 'Gr33nG0Blin']);
$v->rules([
    'requiredWith' => [
        ['password', 'username']
    ]
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['first_name' => 'steve', 'last_name' => 'holt', 'suffix' => 'Mr']);
$v->rules([
    'requiredWith' => [
...
```php
$v = Sulis::validator()->make(['first_name' => 'steve']);
$v->rules([
    'requiredWith' => [
        ['suffix', ['first_name', 'last_name'], true]
    ]
...
$v = Sulis::validator()->make(['username' => 'spiderman']);
$v->rules([
    'requiredWithout' => [
        ['username', 'first_name']
    ]
...
$v = Sulis::validator()->make(['username' => 'spiderman', 'first_name' => 'Peter']);
$v->rules([
    'requiredWithout' => [
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['username' => 'BatMan']);
$v->rules([
    'requiredWithout' => [
...
```php
$v = Sulis::validator()->make(['first_name' => 'steve', 'last_name' => 'holt']);
$v->rules([
    'requiredWithout' => [
        ['suffix', ['first_name', 'last_name'], true]
    ]
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['password' => 'youshouldnotseethis', 'confirmPassword' => 'youshouldnotseethis']);
$v->rules([
    'equals' => [
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['remember_me' => true]);
$v->rules([
    'accepted' => [
        ['remember_me']
    ]
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['amount' => 3.14]);
$v->rules([
    'numeric' => [
        ['amount']
    ]
...
```php
$v = Sulis::validator()->make(['negative' => '-27', 'positive'=>'27']);
...
Alternate Syntax.
```php
$v = Sulis::validator()->make(['user_notifications' => ['bulletin_notifications' => true, 'marketing_notifications' => false, 'message_notification' => true]]);
$v->rules([
    'array' => [
        ['user_notifications']
    ]
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['username' => 'bobburgers']);
$v->rules([
    'length' => [
        ['username', 10]
    ]
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['username' => 'martha']);
$v->rules([
    'lengthMin' => [
        ['username', 5]
    ]
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['username' => 'bruins91']);
$v->rules([
    'lengthMax' => [
        ['username', 10]
    ]
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['color' => ['blue', 'green', 'red', 'yellow']]);
$v->rules([
    'listContains' => [
        ['color', 'yellow']
    ]
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['color' => 'purple']);
$v->rules([
    'in' => [
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['user_ip' => '127.0.0.1']);
$v->rules([
    'ip' => [
        ['user_ip']
    ]
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['user_ip' => '0:0:0:0:0:0:0:1']);
$v->rules([
    'ipv6' => [
        ['user_ip']
    ]
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['user_email' => 'someone@example.com']);
$v->rules([
    'email' => [
        ['user_email']
    ]
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['user_email' => 'some_fake_email_address@gmail.com']);
$v->rules([
    'emailDNS' => [
        ['user_email']
    ]
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['website' => 'https://example.com/contact']);
$v->rules([
    'url' => [
        ['website']
    ]
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['username' => 'batman123']);
$v->rules([
    'alphaNum' => [
        ['username']
    ]
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['username' => 'L337-H4ckZ0rz_123']);
$v->rules([
    'slug' => [
        ['username']
    ]
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['created_at' => '2018-10-13']);
$v->rules([
    'date' => [
        ['created_at']
    ]
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['created_at' => '2018-09-01']);
$v->rules([
    'dateBefore' => [
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['colors' => ['green', 'blue']]);
$v->rules([
    'subset' => [
...
```php
$v = Sulis::validator()->make(['colors' => ['purple', 'blue']]);
$v->rules([
    'subset' => [
        ['colors', ['orange', 'green', 'blue', 'red']]
    ]
...
```php
$v = Sulis::validator()->make(['colors' => ['purple', 'purple']]);
$v->rules([
    'containsUnique' => [
        ['colors']
    ]
...
Alternate syntax.
```php
$v = Sulis::validator()->make(['date' => new \DateTime()]);
$v->rules([
    'instanceOf' => [
        ['date', 'DateTime']
    ]
...
```php
$v = Sulis::validator()->make(array("foo" => "bar"));
...
```php
$v = Sulis::validator()->make(['email_address' => 'test@test.com']);
...
$v = Sulis::validator()->make(array('foo' => 'bar', 'bar' => 1));
$v->rules($rules);
$v->validate();
```
...
$v = Sulis::validator()->make(array('foo' => 'bar', 'bar' => 'mail@example.com));
